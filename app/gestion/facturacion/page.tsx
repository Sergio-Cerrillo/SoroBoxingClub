...
import { DollarSign, TrendingUp, CheckCircle2, AlertCircle, CreditCard, Wallet, Calendar, Users } from "lucide-react"
...
export default function FacturacionPage() {
  ...
  const monthlyRevenue = activeUsers.length > 0 && activeUsers.some(u => u.dues && u.dues.length > 0)
    ? Math.round(activeUsers.reduce((sum, u) => {
  ...
  const paymentMethods = [
    { name: "Efectivo", count: Math.floor(stats.paidCount * 0.4), amount: Math.floor(stats.totalRevenue * 0.4), color: "bg-green-500" },
    { name: "Tarjeta", count: Math.floor(stats.paidCount * 0.35), amount: Math.floor(stats.totalRevenue * 0.35), color: "bg-blue-500" },
    { name: "Transferencia", count: Math.floor(stats.paidCount * 0.25), amount: Math.floor(stats.totalRevenue * 0.25), color: "bg-purple-500" },
  ]
      ...
      <section className="pt-32 pb-12 bg-gradient-to-b from-primary to-primary/95">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl md:text-6xl font-bold text-primary-foreground mb-4 text-center">
            <span className="text-accent">FACTURACIÓN</span> Y COBROS
          </h1>
          <p className="text-lg text-primary-foreground/80 font-mono text-center">
            Control financiero del {months[selectedMonth]} {selectedYear}
          </p>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            <Card className="p-6 border-2 border-accent/30 bg-gradient-to-br from-accent/5 to-transparent">
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge className="bg-accent/20 text-accent border-accent/30">Mes actual</Badge>
              </div>
              <p className="text-sm text-muted-foreground font-mono mb-1">INGRESOS COBRADOS</p>
              <p className="text-3xl font-bold text-accent mb-2">{stats.totalRevenue}€</p>
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <TrendingUp className="w-4 h-4 text-green-600" />
                <span>
                  {stats.collectionRate.toFixed(0)}% del total esperado ({stats.totalExpected}€)
                </span>
              </div>
            </Card>
            ...
            <Card className="p-6 border-2 border-green-500/20">
              ...
              <p className="text-sm text-muted-foreground font-mono mb-1">PAGOS RECIBIDOS</p>
              <p className="text-3xl font-bold text-green-600 mb-2">{stats.paidCount}</p>
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <Calendar className="w-4 h-4" />
                <span>
                  {stats.paidOnTime} a tiempo / {stats.latePayments} con retraso
                </span>
              </div>
            </Card>
            ...
            <Card className="p-6 border-2 border-destructive/20">
              ...
              <p className="text-sm text-muted-foreground font-mono mb-1">COBROS PENDIENTES</p>
              <p className="text-3xl font-bold text-destructive mb-2">{stats.unpaidCount}</p>
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <DollarSign className="w-4 h-4" />
                <span>{stats.pendingRevenue}€ por cobrar</span>
              </div>
            </Card>
            ...
            <Card className="p-6 border-2 border-primary/20">
              ...
              <p className="text-sm text-muted-foreground font-mono mb-1">USUARIOS ACTIVOS</p>
              <p className="text-3xl font-bold mb-2">{activeUsers.length}</p>
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <TrendingUp className="w-4 h-4" />
                <span>Cuota mensual: {monthlyRevenue}€</span>
              </div>
            </Card>
          </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      <p className="text-2xl font-bold text-accent">{monthlyRevenue}€</p>
                      <p
                    ...
                    {!payment.paid && (
                      <Button
                        onClick={() => handleMarkAsPaid(u.id)}
                        className="bg-accent hover:bg-accent/90 text-accent-foreground"
                      >
                        Marcar Pagado
                      </Button>
                    )}
                    {payment.paid && <CheckCircle2 className="w-8 h-8 text-green-600" />}
                  </div>
                </div>
              ))}
            </div>
